...
class TestInjectAttentionHeadMask:
    ...
    def test_same_as_nullify(self, bert_test_model: PreTrainedModel, random_input_batch) -> None:
        ...
        with torch.no_grad():
            ...
            # set params
            heads_to_mask = {0: [0]}
            head_mask = torch.ones((2, 4), device=bert_test_model.device)
            ...
            masking_handles = inject_attention_head_mask(bert_test_model, head_mask)
            masked_last_hidden_state = bert_test_model(random_input_batch["input_ids"], random_input_batch["attention_mask"])[0]
            ...
            for handle in masking_handles:
                handle.remove()
            no_handles_last_hidden_state = bert_test_model(random_input_batch["input_ids"], random_input_batch["attention_mask"])[0]
...
class TestInjectAttentionLayerMask:
    ...
    def test_same_as_nullify(self, bert_test_model: PreTrainedModel, random_input_batch) -> None:
        ...
        with torch.no_grad():
            ...
            # set params
            layers_to_mask = [0]
            layer_mask = torch.ones((2,), device=bert_test_model.device)
            ...
            # mask parts of the model
            masking_handles = inject_attention_layer_mask(bert_test_model, layer_mask)
            masked_last_hidden_state = bert_test_model(random_input_batch["input_ids"], random_input_batch["attention_mask"])[0]
...
class TestInjectFfnNeuronMask:
    ...
    def test_same_as_nullify(self, bert_test_model: PreTrainedModel, random_input_batch) -> None:
        ...
        with torch.no_grad():
            ...
            neuron_mask = torch.ones((2, bert_test_model.config.intermediate_size), device=bert_test_model.device)
            ...
            # mask parts of the model
            masking_handles = inject_ffn_neuron_mask(bert_test_model, neuron_mask)
            masked_last_hidden_state = bert_test_model(random_input_batch["input_ids"], random_input_batch["attention_mask"])[0]
...
class TestInjectFfnLayerMask:
    ...
    def test_same_as_nullify(self, bert_test_model: PreTrainedModel, random_input_batch) -> None:
        ...
        with torch.no_grad():
            ...
            # mask parts of the model
            masking_handles = inject_ffn_layer_mask(bert_test_model, layer_mask)
            masked_last_hidden_state = bert_test_model(random_input_batch["input_ids"], random_input_batch["attention_mask"])[0]
