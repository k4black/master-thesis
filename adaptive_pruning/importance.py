...
def collect_mask_gradients(
    ...
    # create masks
    attention_head_mask = torch.ones(config.num_hidden_layers, config.num_attention_heads).to(device=model.device, dtype=model.dtype, non_blocking=True)
    ffn_neuron_mask = torch.ones(config.num_hidden_layers, config.intermediate_size).to(device=model.device, dtype=model.dtype, non_blocking=True)
    ffn_layer_mask = torch.ones(config.num_hidden_layers).to(device=model.device, dtype=model.dtype, non_blocking=True)
    attention_layer_mask = torch.ones(config.num_hidden_layers).to(device=model.device, dtype=model.dtype, non_blocking=True)
    ...
    attention_head_grads = torch.empty((0, config.num_hidden_layers, config.num_attention_heads)).to(model.device, dtype=model.dtype, non_blocking=True)
    attention_layer_grads = torch.empty((0, config.num_hidden_layers)).to(model.device, dtype=model.dtype, non_blocking=True)
    ffn_neuron_grads = torch.empty((0, config.num_hidden_layers, config.intermediate_size)).to(model.device, dtype=model.dtype, non_blocking=True)
...
def collect_activations(
    ...
    # Collect activations
    attention_head_activations = torch.empty((0, config.num_hidden_layers, config.num_attention_heads)).to(model.device, dtype=model.dtype, non_blocking=True)
    attention_layer_activations = torch.empty((0, config.num_hidden_layers)).to(model.device, dtype=model.dtype, non_blocking=True)
    ffn_neuron_activations = torch.empty((0, config.num_hidden_layers, config.intermediate_size)).to(model.device, dtype=model.dtype, non_blocking=True)
    ffn_layer_activations = torch.empty((0, config.num_hidden_layers)).to(model.device, dtype=model.dtype, non_blocking=True)
    hidden_state_activations = torch.empty((0, config.hidden_size)).to(model.device, dtype=model.dtype, non_blocking=True)
    meta_activations = torch.empty((0, 5)).to(model.device, dtype=model.dtype, non_blocking=True)

    current_attention_head_activations = torch.empty((0, 0, config.num_attention_heads)).to(model.device, dtype=model.dtype, non_blocking=True)
    current_attention_layer_activations = torch.empty((0, 0)).to(model.device, dtype=model.dtype, non_blocking=True)
    current_ffn_neuron_activations = torch.empty((0, 0, config.intermediate_size)).to(model.device, dtype=model.dtype, non_blocking=True)
    current_ffn_layer_activations = torch.empty((0, 0)).to(model.device, dtype=model.dtype, non_blocking=True)
    current_hidden_state_activations = torch.empty((0, 0, config.hidden_size)).to(model.device, dtype=model.dtype, non_blocking=True)
    ...
    for layer_idx, layer in enumerate(model.model.layers):
        def hook_fn_attention(module: torch.nn.Module, input: torch.Tensor | tuple[torch.Tensor], output: torch.Tensor | None = None):
            ...
            average_over_heads = average_over_sequence.view(-1, config.num_attention_heads, config.hidden_size // config.num_attention_heads).mean(dim=2)
            # append to current_activations on the layers dimension (1)
            current_attention_head_activations = torch.cat([current_attention_head_activations, average_over_heads.unsqueeze(1)], dim=1)
            current_attention_layer_activations = torch.cat([current_attention_layer_activations, average_over_heads.mean(dim=1).unsqueeze(1)], dim=1)

        def hook_fn_ffn(module: torch.nn.Module, input: torch.Tensor | tuple[torch.Tensor], output: torch.Tensor | None = None):
            ...
            average_over_sequence = input[0].mean(dim=1)
            current_ffn_neuron_activations = torch.cat([current_ffn_neuron_activations, average_over_sequence.unsqueeze(1)], dim=1)
            current_ffn_layer_activations = torch.cat([current_ffn_layer_activations, average_over_sequence.mean(dim=1).unsqueeze(1)], dim=1)

        def hook_fn_hidden_states(module: torch.nn.Module, input: torch.Tensor | tuple[torch.Tensor], output: torch.Tensor | None = None):
            # output: [batch_size, seq_len, hidden_size]
            # average over sequence length: [batch_size, hidden_size]
            nonlocal current_hidden_state_activations
            average_over_sequence = output[0].mean(dim=1)
            current_hidden_state_activations = torch.cat([current_hidden_state_activations, average_over_sequence.unsqueeze(1)], dim=1)
    ...
    for batch in batch_iterator:
        ...
        current_attention_head_activations = torch.zeros((batch_size, 0, config.num_attention_heads)).to(model.device, dtype=model.dtype, non_blocking=True)
        current_attention_layer_activations = torch.zeros((batch_size, 0)).to(model.device, dtype=model.dtype, non_blocking=True)
        current_ffn_neuron_activations = torch.zeros((batch_size, 0, config.intermediate_size)).to(model.device, dtype=model.dtype, non_blocking=True)
        current_ffn_layer_activations = torch.zeros((batch_size, 0)).to(model.device, dtype=model.dtype, non_blocking=True)
        current_hidden_state_activations = torch.zeros((batch_size, 0, config.hidden_size)).to(model.device, dtype=model.dtype, non_blocking=True)

        # forward pass
        _ = model(**batch)

        # cat current_activations to activations
        attention_head_activations = torch.cat([attention_head_activations, current_attention_head_activations.detach()], dim=0)
        attention_layer_activations = torch.cat([attention_layer_activations, current_attention_layer_activations.detach()], dim=0)
        ...
        hidden_state_activations = torch.cat([hidden_state_activations, current_hidden_state_activations.detach().mean(dim=1)], dim=0)
    ...
    ], dim=1).to(model.device, dtype=model.dtype, non_blocking=True)
...
def collect_weight_magnitudes(
    ...
    for layer_idx, layer in enumerate(model.model.layers):
        ...
        mean_key_weight = layer.self_attn.k_proj.weight.mean(dim=1).view(config.num_key_value_heads, -1).mean(dim=1).repeat_interleave(num_heads_per_key_value_group)
        mean_value_weight = layer.self_attn.v_proj.weight.mean(dim=1).view(config.num_key_value_heads, -1).mean(dim=1).repeat_interleave(num_heads_per_key_value_group)
