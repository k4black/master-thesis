...
df['pruning_components'] = df['method'].apply(lambda x: x.replace('+random', '').replace('+l1', '').replace('+l2', '').replace('+taylor', '').replace('+fisher_info', '').replace('+fixed_x05_x2', '').replace('+fixed_x2_x05', '').replace('+fixed', '').replace('+relative_per_param', '').replace('+relative', '').replace('+relative_per_param', '').replace('+meta', ''))
...
df = df.sort_values(by=['line_name'], key=lambda x: -2*x.str.contains('Original').astype(int)-x.str.lower().str.contains('our').astype(int), ignore_index=True)
...
df_for_latex = df[['lib', 'method', 'Ratio (\%)', '#Parameters (1e9)', 'GFLOPs', 'wikitext', 'piqa', 'boolq', 'arc_easy']]
